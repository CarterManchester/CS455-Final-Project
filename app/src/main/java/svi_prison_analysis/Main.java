...
public class Main {
    ...
    public static void main(String[] args) {
        SparkSession spark = SparkSession.builder()
                ...
                .master("local[*]") // IMPORTANT!! LOCAL MODE! Comment out (or change?) when testing on HDFS
                                    // cluster!
                .getOrCreate();

        spark.sparkContext().setLogLevel("WARN");
        spark.conf().set("spark.sql.debug.maxToStringFields", "200"); // NOTE: This is so it prints strings without size
                                                                      // warnings. Change back if needed!!
     ...
     */
    private static void mostVulnerableCountiesAcrossStates(int n, State... states) {// no way this 'State... states'
        ...
        for (State state : stateList) {
            Dataset<Row> temp = state.getsviData().select(
                    col("STATE"),
                    col("COUNTY"),
                    col("FIPS"),
                    col("RPL_THEMES"),
